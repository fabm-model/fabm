...
!BOC
   select case (solver)
      ...
      case (1003)
         call runge_kutta_4_ppdd(dt,numc,nlev,cc,right_hand_side_ppdd)
      case default
...
!
! !INTERFACE:
   subroutine runge_kutta_4_ppdd(dt,numc,nlev,cc,right_hand_side)
!
! !DESCRIPTION:
! Here, the fourth-order Runge-Kutta (RK4) scheme is coded, 
! with four evaluations
...
! !INPUT/OUTPUT PARAMETER:
   ...
   interface
      subroutine right_hand_side(first,numc,nlev,cc,pp,dd)
         ...
         integer, intent(in)                  :: numc,nlev
         REALTYPE, intent(in)                 :: cc(1:numc,0:nlev)
         REALTYPE, intent(out)                :: pp(1:numc,1:numc,0:nlev),dd(1:numc,1:numc,0:nlev)
      end
   end interface
!
! !REVISION HISTORY:
...
!  absolutely essential since not all elements are calculated 
   ...
   return
   end subroutine runge_kutta_4_ppdd
!EOC
