...
!-----------------------------------------------------------------------
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! MODIFIED BY D. MACIAS AT JRC TO ADAPT TO THE MED SEA; START DATE JUNE-2013
! FURTHER CONVERTED INTO MEDERGOM BY D. MACIAS BY ADDING A SECOND ZOOPLANKTON
! COMPARTMENT JUNE 2014
!-----------------------------------------------------------------------
!BOP
!
...
! !PUBLIC_DERIVED_TYPES:
  type,extends(type_base_model),public :: type_jrc_med_ergom
      ...
      real(rk) :: imin_di,imin_fla,imin_cya,kc,q10_rec,ade_r0,alphaade,q10_recs
...
!BOC
   ...
   call self%get_parameter(self%imin_fla,'imin_fla', 'W/m2', 'minimal optimal light radiation, flagellates', default=50._rk)
   call self%get_parameter(self%imin_cya,'imin_cya', 'W/m2', 'minimal optimal light radiation, cyanobacterias', default=50._rk)
   call self%get_parameter(self%kc,      'kc',      'm2/mmol n', 'attenuation constant for self-shading effect', default=0.03_rk)
...
!
! !INPUT PARAMETERS:
   class(type_jrc_med_ergom), INTENT(IN) :: self
  _DECLARE_ARGUMENTS_DO_
!
! !LOCAL VARIABLES:
    real(rk)           :: pp,ff,bb,zz,aa,nn,po,dd,o2,pw,par,I_0,zz0,ntemp,ntemp_eps,ptemp,phyto
    real(rk)           :: ppg,ffg,bbg,nlim,plim,rp,rf,rb
    real(rk)           :: iopt_fla,iopt_cya,iopt_di,ppi_fla,ppi_cya,ppi_di,temp,tempq,salt,ldn,nf,lpn,lpd,lp
...
!BOC
   ...
   iopt_fla = max(0.5_rk*par,self%imin_fla)
   ppi_fla = par/iopt_fla*exp(1.0_rk-par/iopt_fla)

   iopt_cya = max(0.5_rk*par,self%imin_cya)
   ppi_cya = par/iopt_cya*exp(1.0_rk-par/iopt_cya)
...
!    end if
   ...
   rf = min(nlim, plim, ppi_fla) ! Liebig's minimum law
   ...
   rb = min(plim, ppi_cya) !Liebig's law: IP,light. No dependence on IN for cyanos.
...
!   _SET_DIAGNOSTIC_(self%id_tot_chla, (pp * 9.5 + 2.1)/self%Yc_diat + (ff * 9.5 + 2.1)/self%Yc_flag + (bb * 9.5 + 2.1)/self%Yc_cyan)
   _SET_DIAGNOSTIC_(self%id_tot_chla, (pp) + (ff)  + (bb)) ! Just assuming a constant factor of 1 (good for the Med)
