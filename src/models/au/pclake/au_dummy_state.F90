...
! !PUBLIC DERIVED TYPES:
   type, extends(type_base_model),public :: type_au_pclake_dummy_state
...
!     Model procedures
   ...
   end type type_au_pclake_dummy_state
...
!
! !INPUT PARAMETERS:
      class (type_au_pclake_dummy_state), intent(inout), target  :: self
...
!     create namelist
!-------------------------------------------------------------------------------
      namelist /au_pclake_dummy_state/ sNH4W_initial,sNO3W_initial,sPO4W_initial,sPAIMW_initial,sSiO2W_initial,sO2W_initial, &
...
!-------------------------------------------------------------------------------
      if (configunit>0) read(configunit,nml=au_pclake_dummy_state,err=99,end=100)
!-------------------------------------------------------------------------------
...
99 call self%fatal_error('au_pclake_dummy_state_init','Error reading namelist au_pclake_dummy_state')

100 call self%fatal_error('au_pclake_dummy_state_init','Namelist au_pclake_dummy_state was not found.')
!
...
!------------------------------------------------------------------------------
   end module au_pclake_dummy_state
