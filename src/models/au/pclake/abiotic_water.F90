...
! !LOCAL VARIABLES:
   ...
   real(rk)          :: cExtSpDet
!   create namelist
   namelist /au_pclake_abiotic_water/ sNH4W_initial,sNO3W_initial,sPO4W_initial,sPAIMW_initial,sSiO2W_initial,sO2W_initial, & !background_initial,
...
!  Read parameters namelist
   if (configunit>0) read(configunit,nml=au_pclake_abiotic_water,err=99,end=100)
...
99 call self%fatal_error('au_pclake_abiotic_water_init','Error reading namelist au_pclake_abiotic_water')

100 call self%fatal_error('au_pclake_abiotic_water_init','Namelist au_pclake_abiotic_water was not found.')
